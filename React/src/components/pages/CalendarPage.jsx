    ...
    <div style={{
      ...
      <Calendar
        ...
        components={{
          ...
          ), // 커스텀 툴바 컴포넌트 연결 및 onView, onNavigate 전달
        }}
        ...
        eventPropGetter={(event) => {
          if (user && event.resource) { // 로그인 상태면서 moodScore가 있을 때만 스타일 적용
            let backgroundColor = '#3174ad'; // 기본 색상
            if (event.resource >= 70) {
              backgroundColor = '#4CAF50'; // 긍정 (초록)
            } else if (event.resource >= 40) {
              backgroundColor = '#FFC107'; // 중립 (주황)
            } else {
              backgroundColor = '#F44336'; // 부정 (빨강)
            }
            return { style: { backgroundColor } };
          }
          return {}; // 비로그인 또는 moodScore 없을 시 기본 스타일
        }}
      />
    </div>
  );
