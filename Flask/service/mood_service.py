...
class MoodDiaryService:
    ...
    def diary(diary_text):
        ...
        try: # AI LLM 모델 호출
            ...
            result_ai = two_stage_mood_classification(diary_text)
            result_dict = extract_mood_with_dict(diary_text)

            polarity = result_ai.get("polarity", 0)
            labels = result_ai.get("label", []) # 지금 tuple 형태로 나오고 있으므로 수정 필요
            tags = result_dict.get("tags", []) # 감성사전을 이용한 태그 도출
            # === tags.py, utils.py를 이용한 custom tag 도출 ===
            # tags = make_tags_prob_and_map(detailed_moods, mood_to_tags_map, threshold=0.1)
            # sentiment_probs = result.get("sentiment_probs", {})
            # top3_moods = tuple([emo for emo, _ in labels[:3]])
